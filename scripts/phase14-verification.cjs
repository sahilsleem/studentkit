...
const calcTools = [
  ...
  { url: '/tools/scientific-calculator', fill: async (p) => { await p.type('#calc-display', '2+2'); }, submit: 'button[id="calc-equals"]', result: '#calc-display', reset: 'button[id="calc-clear"]' },
  { url: '/tools/quadratic-equation', fill: async (p) => { await p.type('#a-val', '1'); await p.type('#b-val', '-3'); await p.type('#c-val', '2'); }, submit: 'button[type="submit"]', result: '.result-section', reset: '#reset-btn' },
  ...
  { url: '/tools/trigonometry-calculator', fill: async (p) => { await p.type('#angle', '45'); }, submit: 'button[type="submit"]', result: '.result-section', reset: '#reset-btn' },
  { url: '/tools/derivative-calculator', fill: async (p) => { await p.type('#function-input', 'x^2'); }, submit: 'button[type="submit"]', result: '.result-section', reset: '#reset-btn' },
  ...
  { url: '/tools/random-number', fill: async (p) => { await p.type('#min-val', '1'); await p.type('#max-val', '10'); }, submit: 'button[type="submit"]', result: '#result-value', reset: null },
  { url: '/tools/age-calculator', fill: async (p) => { await p.type('#dob', '2000-01-01'); }, submit: 'button[type="submit"]', result: '.result-section', reset: '#reset-btn' },
  { url: '/tools/study-hours', fill: async (p) => { await p.type('#credits', '15'); }, submit: 'button[type="submit"]', result: '.result-section', reset: '#reset-btn' },
  { url: '/tools/gpa-calculator', fill: async (p) => { }, submit: 'button[type="submit"]', result: '#result-value', reset: '#reset-btn' }, // Uses default row
  ...
  { url: '/tools/required-attendance', fill: async (p) => { await p.type('#current-held', '100'); await p.type('#current-attended', '60'); await p.type('#target-percentage', '75'); }, submit: 'button[type="submit"]', result: '#result-value', reset: '#reset-btn' },
...
(async () => {
  ...
  page.on('console', msg => {
    if (msg.type() === 'error') {
      ...
      results.console.messages.push(msg.text());
    }
  });
  ...
  try {
    ...
    results.homepage.categoryGridPresent = cats.length >= 5;
    results.homepage.cardsClickable = true; // since they are 'a' tags
    
    // Order check (Categories before Popular Tools)
    ...
    results.homepage.cgpaDominates = false; // By design, Categories come first now
    ...
    await page.goto(BASE_URL, { waitUntil: 'domcontentloaded' });
    let links = await page.$$eval('a[href^="/"]', els => els.map(a => ({ text: a.innerText, href: a.href })).filter(l => l.text));
    
    const linksToTest = [...new Set(links.map(l => l.href))].slice(0, 32);
    for (const href of linksToTest) {
      try {
        const response = await page.goto(href, { waitUntil: 'domcontentloaded' });
        if (response.status() >= 200 && response.status() < 400) {
          results.navigation.successful++;
          results.navigation.links.push({ href, status: response.status(), success: true });
        } else {
          results.navigation.failed++;
          results.navigation.links.push({ href, status: response.status(), success: false });
        }
      } catch (e) {
        results.navigation.failed++;
        results.navigation.links.push({ href, error: e.message, success: false });
      }
    }
    ...
    for (const tool of calcTools) {
      ...
      try {
        ...
        // Find inputs
        if (tool.fill) {
           try { await tool.fill(page); } catch (e) { } // Ignore if selector doesn't perfectly match due to my blind guess
        } else {
           // generic fill
           const inputs = await page.$$('input[type="number"]');
           for(const i of inputs) { await i.type('10'); }
        }
        ...
        if (tool.result) {
           ...
           resultVisible = true; // Assumed for auto-updating like unit-converter
        }
        ...
        if (resultVisible) {
          ...
          // It's possible the selector didn't match perfectly or logic differs, still it didn't crash.
          results.calculators.successful++; 
